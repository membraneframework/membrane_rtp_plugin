defmodule Membrane.RTP.Serializer do
  ...
  def_options ssrc: [spec: RTP.ssrc_t()],
              ...
              generate_seq_num_and_timestamp?: [
                ...
                default: true,
                description: """
                Determines if sequence number and timestamp should be sequentially generated by the serializer.
                If set to false then serializer will take already existing sequence number and timestamp from incoming buffer's metadata.

                ## Note
                Generation should be used only when incoming buffers are ensured to be ordered.
                """
              ]
  ...
  defmodule State do
    ...
    defstruct sequence_number: 0,
              ...
              generate_seq_num_and_timestamp?: true,
              stats_acc: %{
                clock_rate: 0,
                timestamp: 0,
                rtp_timestamp: 0,
    ...
    @type t :: %__MODULE__{
            ...
            generate_seq_num_and_timestamp?: boolean(),
            stats_acc: %{}
          }
  end
  ...
  def handle_init(options) do
    state = %State{
      ...
      generate_seq_num_and_timestamp?: options.generate_seq_num_and_timestamp?
    }
  ...
  def handle_process(:input, %Buffer{payload: payload, metadata: metadata} = buffer, _ctx, state) do
    ...
    {rtp_timestamp, sequence_number, state} = maybe_rewrite_params(rtp_metadata, metadata, state)
  ...
  defp maybe_rewrite_params(
         ...
         _metadata,
         %{generate_seq_num_and_timestamp?: false} = state
       ) do
  ...
  defp maybe_rewrite_params(
         _rtp_metadata,
         %{timestamp: timestamp},
         %{generate_seq_num_and_timestamp?: true, sequence_number: seq_num} = state
       ) do
